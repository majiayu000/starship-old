...
渲染时可提供以下选项：
- **mergeConsecutiveStrings**: 是否合并连续字符串
- **convertOldRootRadicand**: 是否转换旧式根号
- **enableParagraphGrouping**: 是否启用段落分组
- **showSpecialSymbols**: 是否显示特殊符号

## 6. 数据流程

1. 内容数据（字符串或对象数组）传入 `useContentAdapter`
2. 根据选项进行预处理（合并字符串、转换特殊格式）
3. 执行段落化处理，生成段落结构
4. 返回处理后的内容
